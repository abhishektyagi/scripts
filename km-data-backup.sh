...
dbFile=/var/mongo/backup/km-data-db.log
...
backup_archive()
{
      ...
      `/usr/share/fk-3p-mongodb-3-0-x/bin/mongodump --host 10.32.5.17 --port 27300 --db $db_name --excludeCollection event --username kmread --password kmRead123 --out $user_dir/$dir_name` >> $logFile 2>&1
      COUNTER=`echo $?` 
...
onFailSendMail()
{
	...
	if [ $val -ge 0 ]
	then
        echo "$msg" | /usr/bin/mail -s "KM Mongo Backup failed" deepanshu.saxena@flipkart.com
...
onSuccessSendMail()
{
    ...
    echo "DB Name $msg" | /usr/bin/mail -s "KM Mongo Backup Successfull" deepanshu.saxena@flipkart.com
   # exit 0	
